...
class JWT
{
     ...
     */
    public static function encodee($payload, $key, $alg = 'SHA256', $keyId = null, $head = null)
    {
     ...
     */
    public static function sign($msg, $key, $alg = 'SHA256')
    {
     ...
     */
    private static function verifyToken($msg, $signature, $key, $alg)
    {
        if (empty(static::$supported_algs[$alg])) {
            throw new DomainException('Algorithm not supported');
        }
    ...
    /**
     * Function to encode the data present with param passed
     *
     * @param string $email
     *
     * @param string $secretkey group of characters
     *
     * @return string A encoded string
     */
    ...
    public function verify($token): bool
    {
        ...
        $rawSignature                                            = hash_hmac('sha256', $dataEncoded, 'prashant', true);
